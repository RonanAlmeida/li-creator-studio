    ...
    <div>
      ...
      >
        <input
          ref={inputRef}
          type="file"
          accept={FILE_LIMITS.video.allowedTypes.map(t => `.${t}`).join(',')}
          onChange={handleChange}
          className="hidden"
        />
        <VideoIcon className="w-12 h-12 mx-auto mb-4 text-linkedin-gray-600" />
        <p className="text-sm font-medium text-linkedin-gray-900 mb-2">
          Drag and drop your video here
        </p>
        <p className="text-xs text-linkedin-gray-600 mb-4">
          or click to browse (MP4, MOV, AVI • Max {FILE_LIMITS.video.maxSize}MB)
        </p>
        <Button
          variant="secondary"
